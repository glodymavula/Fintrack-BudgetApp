...
class DatabaseHelper(private val context: Context) :
    ...
    companion object {
        ...
        private const val COLUMN_BUDGET_GOAL_ID = "id"
        private const val COLUMN_BUDGET_GOAL_USER_ID = "user_id"
        private const val COLUMN_BUDGET_GOAL_CATEGORY = "category"
        private const val COLUMN_BUDGET_GOAL_MIN = "min_budget"
        private const val COLUMN_BUDGET_GOAL_MAX = "max_budget"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("""
            CREATE TABLE $TABLE_BUDGET_GOALS (
                $COLUMN_BUDGET_GOAL_ID INTEGER PRIMARY KEY AUTOINCREMENT,
                $COLUMN_BUDGET_GOAL_USER_ID INTEGER,
                $COLUMN_BUDGET_GOAL_CATEGORY TEXT,
                $COLUMN_BUDGET_GOAL_MIN REAL,
                $COLUMN_BUDGET_GOAL_MAX REAL,
                FOREIGN KEY($COLUMN_BUDGET_GOAL_USER_ID) REFERENCES $TABLE_USERS($COLUMN_USER_ID)
            )
        """.trimIndent())
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) {
            // Add budget goals table in version 2
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS $TABLE_BUDGET_GOALS (
                    $COLUMN_BUDGET_GOAL_ID INTEGER PRIMARY KEY AUTOINCREMENT,
                    $COLUMN_BUDGET_GOAL_USER_ID INTEGER,
                    $COLUMN_BUDGET_GOAL_CATEGORY TEXT,
                    $COLUMN_BUDGET_GOAL_MIN REAL,
    ...
    fun addBudgetGoal(budgetGoal: BudgetGoal): Long {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_BUDGET_GOAL_USER_ID, budgetGoal.userId)
            put(COLUMN_BUDGET_GOAL_CATEGORY, budgetGoal.category)
            put(COLUMN_BUDGET_GOAL_MIN, budgetGoal.minBudget)
            put(COLUMN_BUDGET_GOAL_MAX, budgetGoal.maxBudget)
        }
        return db.insert(TABLE_BUDGET_GOALS, null, values)
    }

    fun getBudgetGoalsByUserId(userId: Int): List<BudgetGoal> {
        ...
        if (cursor.moveToFirst()) {
            do {
                budgetGoals.add(
                    BudgetGoal(
                        id = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_BUDGET_GOAL_ID)),
                        userId = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_BUDGET_GOAL_USER_ID)),
                        category = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_BUDGET_GOAL_CATEGORY)),
                        minBudget = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_BUDGET_GOAL_MIN)),
                        maxBudget = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_BUDGET_GOAL_MAX))
                    )
                )
            } while (cursor.moveToNext())
        }
    ...
    fun updateBudgetGoal(budgetGoal: BudgetGoal): Boolean {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_BUDGET_GOAL_CATEGORY, budgetGoal.category)
            put(COLUMN_BUDGET_GOAL_MIN, budgetGoal.minBudget)
        ...
        val rowsUpdated = db.update(
            TABLE_BUDGET_GOALS,
            values,
            "$COLUMN_BUDGET_GOAL_ID = ? AND $COLUMN_BUDGET_GOAL_USER_ID = ?",
            arrayOf(budgetGoal.id.toString(), budgetGoal.userId.toString())
        )
        return rowsUpdated > 0
    }

    fun deleteBudgetGoal(goalId: Int, userId: Int): Boolean {
        val db = writableDatabase
        val rowsDeleted = db.delete(
            TABLE_BUDGET_GOALS,
            "$COLUMN_BUDGET_GOAL_ID = ? AND $COLUMN_BUDGET_GOAL_USER_ID = ?",
            arrayOf(goalId.toString(), userId.toString())
        )
        return rowsDeleted > 0
    }
}
